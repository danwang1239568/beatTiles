...
const scene1 = {
  ...
  create() {
    ...
    // 生成粒子
    let currentX
    this.particle = this.add.particles(keyPositions[currentX ? currentX : 0] + gameWidth * 0.0625, gameHeight * 0.97, 'star', {
      frequency: 1,
      lifespan: 150,
    ...
    const pressedKey = keys.find((k, idx) => this.input.keyboard.addKey(k).isDown);
    if (pressedKey) {
      this.particle.setPosition(keyPositions[currentX], gameHeight * 0.97).start()
    ...
    let key;
    const next = (i) => {
      let cerrentX = Phaser.Math.Between(0, 3);
      ...
      if (currentMusic.musics[i].key !== '0') {
        ...
        const handleHit = (keyItem) => {
          ...
          this.particle.setPosition(keyPositions[currentX], gameHeight * 0.97).start();
          this.time.delayedCall(100, () => this.particle.stop());
          keyItem.destroy();
        }
